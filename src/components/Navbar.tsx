...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav
        className={`bg-gradient-to-b from-white to-gray-100 text-gray-800 h-screen fixed top-0 left-0 z-50 transition-all duration-300 ${isExpanded ? "w-64" : "w-16"} shadow-lg`}
      >
        <div className="flex flex-col h-full">
          <div className="p-4 flex items-center justify-between">
            {isExpanded && (
              <span className="text-xl font-bold text-blue-600">Recall</span>
            )}
            <button
          ...
          <div className="p-4">
            {isExpanded ? (
              <div className="bg-white-50 rounded-lg p-4 mb-4 ">
                <h3 className="text-sm font-semibold text-black-600 mb-1">
                  Available Credits
                </h3>
      ...
      <div
        className={`transition-all duration-300 ${isExpanded ? "ml-64" : "ml-16"}`}
      >
